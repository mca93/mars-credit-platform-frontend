...
export default {
  ...
  methods: {
    ...
    calcDims() {
      ...
      if (!this.isMobile) {
        this.height =
          document.getElementById("table-wrapper").clientHeight - 40;
  ...
  filters: {
    selected: function(value, sel) {
      ...
      return value
        .toString()
        .replace(new RegExp(sel, "gim"), `<span class="sel">${sel}</span>`);
    }
  },
  created() {
